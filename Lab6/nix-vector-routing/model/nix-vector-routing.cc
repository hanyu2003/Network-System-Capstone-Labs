...
//////////////////////////////////////////////
#include <fstream>        // NEW: 讀檔
#include <sstream>        // NEW: 解析行
#include <ns3/log.h>      // NEW: 錯誤列印
...
void
NixVectorRouting<Protocol>::SetPaths (const std::string &pathFile)
{
  std::ifstream file (pathFile);
  if (!file.is_open ())
    {
      NS_LOG_ERROR ("NixVectorRouting::SetPaths(): 無法開啟檔案 " << pathFile);
      return;
    }

  int src, dst, length;
  while (file >> src >> dst >> length)
    {
      std::vector<int> path;
      path.reserve (length);
      ...
      m_table[std::make_pair (src, dst)] = path;
    }
  file.close ();
}
///////////////////////////////////////////////////////////
...
NixVectorRouting<T>::GetNixVector (Ptr<Node> source, IpAddress dest, Ptr<NetDevice> oif) const
{
  ...
  // ==== NEW: Task1.2 手動 path 查找 ====//////////////////////////////////////
  // 1. 從 table 拿 srcId/dstId
  ...
  if (it != m_table.end ())
    {
      NS_LOG_LOGIC ("Use manual path for " << srcId << " -> " << dstId);
      // 把 vector<int> 轉成 parentVector
      std::vector< Ptr<Node> > parentVector;
      parentVector.assign (NodeList::GetNNodes (), 0);
      for (size_t i = 1; i < it->second.size (); ++i)
        {
          int prev = it->second[i-1];
          int curr = it->second[i];
          parentVector.at (curr) = NodeList::GetNode (prev);
        }
      // 交給 BuildNixVector 回傳 nixVector
      if (BuildNixVector (parentVector, srcId, dstId, nixVector))
        {
          return nixVector;
        }
      else
        {
          NS_LOG_ERROR ("BuildNixVector failed for manual path");
          return 0;
        }
    }
  // ==== 手動 path end ====///////////////////////////////////////////////////////
